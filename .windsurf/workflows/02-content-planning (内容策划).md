...
3. **硬性校验（撰写时逐条自查）**：
   ...
   - **选型理由成节**：独立一节讲“为什么选这条路线”并引用 `_decisions/`；不把常规环境坑当主卖点（反 FUD）。
   - **证据状态进正文**：每个结论就地标 `verified`/`paper_spec` 与验收标准。
...
2. **物理文件 B：`README.md` (视频分镜与校验线)**：
   - 提取软文中的核心步骤，转化为 5 段式视频结构大纲（每段明确 **Beat Type** 叙事节奏与 **Visual Priority** 视觉焦点）。分镜须据软文实际主线重新提取（若软文主线变动，不得沿用旧分镜）；画面须映射 `remotion-spec.md` 现成组件代号。
